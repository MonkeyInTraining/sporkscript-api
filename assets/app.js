...
{				
	...
	/*#app*/ this.ChooseFile = function( msg,type,callback,fldr ) { prompt( "#", "App.ChooseFile(\f"+msg+"\f"+type+"\f"+_Cbm(callback)+"\f"+fldr ); }
...
function Web( id )
{
    ...
    /*#web*/ obj.SetUserAgent = function( agent ) { prompt( obj.id, "Web.SetUserAgent(\f"+agent ); } 
...
function _SetTempDebug(sw) { if(_dbg != _dbgSave) app.SetDebug(sw, _dbgSave = _dbg); }
function _RestoreDebug() { if(_dbg != _dbgSave) app.SetDebug(_dbgSave = _dbg); }
...
function _IdeCmd( cmd ) { return prompt( "#", "_IdeCmd(\f"+cmd ); }
