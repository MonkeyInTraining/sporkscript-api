...
Obj.prototype.DrawImage = function( image, x, y, w, h, angle )
{
    x = x * this.width;  
    y = y * this.height;
    if( w && w!=-1 ) w = w * this.width; else w = h * this.height * image.width/image.height;
    if( h && h!=-1 ) h = h * this.height; else h = w * image.height/image.width;
    this.ctx.save();
    ...
    //this.ctx.drawImage( image, 0, 0, image.origWidth, image.origHeight, -(w/2), -(h/2), w, h );
    this.ctx.drawImage( image, 0, 0, image.width, image.height, -(w/2), -(h/2), w, h );
    this.ctx.restore();
}

/*
Obj.prototype.DrawSprite = function( sheet, sx, sy, sw, sh, dx, dy, dw, dh )
{
	dx = dx * this.width;  
    dy = dy * this.height;
    if( dw > -1 ) dw = dw * this.width; else dw = dh * this.height * sw/sh;
    if( dh > -1 ) dh = dh * this.height;  else dh = dw * sh/sw;
	this.ctx.save();
	this.ctx.resetTransform();
...
Obj.prototype.DrawSprite = function ( sheet, sx, sy, sw, sh, dx, dy, dw, dh, angle ) 
{
    dx = dx * this.width;
    dy = dy * this.height;
    if( dw && dw!=-1 ) dw = dw * this.width; else dw = dh * this.height * sw/sh;
    if( dh && dh!=-1 ) dh = dh * this.height; else dh = dw * sh/sw;
    ...
    this.ctx.drawImage( sheet, sx, sy, sw, sh, -(dw/2), -(dh/2), dw, dh );
    this.ctx.restore();
}

Obj.prototype.CreateImage = function( file, callback )
{
    var img = FastCanvas.createImage(); 
    if( callback && callback.name ) img.onload = callback;
    img.src = file; 
    img.Destroy = function() { img.src = null; } 
    return img;
}
...
Obj.prototype.GetContext = function()
{
    return this.ctx;
}
...
function FastCanvasImage() 
{
    ...
    //DH: Not needed if we load using BitmapFactory
